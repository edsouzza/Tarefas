...
public class DAOPatriTensTransferido {
    ...
    public boolean salvarPatriItensTransferidoDAO(PatriTensTransferido pPatritransferido) 
    {
        ...
        try 
        {
            String sql = "INSERT INTO TBLITENSMEMOTRANSFERIDOS (item, numemo, modelo, serie, chapa, destino, status) VALUES (?,?,?,?,?,?,?)";
            ...
            pst.setString(6, pPatritransferido.getDestino());                 
            pst.setString(7, pPatritransferido.getStatus());
            pst.executeUpdate(); 
            return true;
        } catch (Exception e) {
...
//        }
    ...
    public boolean memoAtualTemItensDAO()
    {
        ...
        {            
            conexao.conectar();           
            //sql = "SELECT codigo FROM TBLITENSMEMOTRANSFERIDOS WHERE numemo = '"+pNumemo+"'";           
