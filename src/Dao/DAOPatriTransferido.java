...
public class DAOPatriTransferido {
    ...
    public boolean salvarPatriTransferidoDAO(PatriTransferido pPatritransferido) 
    {
        ...
        try 
        {
            String sql = "INSERT INTO TBLMEMOSTRANSFERIDOS (numemo, datacad, status, observacao) VALUES (?,?,?,?)";
            PreparedStatement pst = conexao.getConnection().prepareStatement(sql);
            pst.setString(1, pPatritransferido.getNumemo());             
            pst.setString(2, dataCadastro);
            pst.setString(3, pPatritransferido.getStatus());
            pst.setString(4, pPatritransferido.getObservacao());
            pst.executeUpdate(); 
            return true;
        } catch (Exception e) {
