...
//        String novaobs  = "\n"+df.format(dataDia)+" : "+pObservacao;
    ...
    public void gravarUpdateMemos(int pCod, String numemo){
        ...
        if(Emicro(pCod))
        {
            ...
            {
                conexao.conectar();
                sql = "UPDATE tblpatrimonios SET estacao=?, ip=?, clienteid=?, status=?, motivo=?, observacoes=?, datainativacao=? WHERE tipoid=1 AND codigo=?";
            ...
            {
                conexao.conectar();
                sql = "UPDATE tblpatrimonios SET ip=?, clienteid=?, status=?, motivo=?, observacoes=?, datainativacao=? WHERE tipoid<>1 AND codigo=?";
